...
# Link capacity: how many cars in transit are allowed between
# neighboring nodes
...
# =============================================================================
# Mesoscopic Functions
# =============================================================================

def outgoing_for(node: Node) -> list:
    ...
    directions = []
    i, j = node.i, node.j  # for ease of use
    if i > 0:
...
def incoming_for(node: Node) -> list:
...
def is_boundary_incoming_link(src: Node, dst: Node) -> bool:
    ...
    di = dst.i  # when == si traffic moves N/S, otherwise E/W
    dj = dst.j  # when == sj traffic moves E/W, otherwise N/S
    return (
...
# =============================================================================
# Macroscopic Setup and Functions
# =============================================================================

# in_transit -- contains the cars currently traveling from node u to node v.
# key -> value: {(u,v) -> deque of (Car, remaining time)}
in_transit = dict()

# stopped -- a queue of cars lining up for one of the neighboring nodes.
# key -> value: {(u,v) -> deque of Cars waiting at light}
stopped = dict()
...
for t in range(TOTAL_TICKS):
    ...
    # Step 1: Move cars along links and push arrivals into downstream queues
    for src, dst in links:
        pop_to_queue_if_arrived(src, dst)
    
    # Step 2: Serve each intersection according to traffic light
    for node in nodes:
        serve_intersection(t, node)
    
    # Step 3: Spawn boundary arrivals
    for src, dst in links:
        if is_boundary_incoming_link(src, dst):
            # Check if we should spawn a car based on arrival rate
            if random.random() < ARRIVAL_RATE:
                # Try to add a car to this boundary link
                new_car = Car(id=car_id, t_enter=t)
                if enqueue_departure(src, dst, new_car):
                    car_id += 1
    
    # Step 4: Update metrics - sample queue lengths
    total_queue_length = 0
    for link in links:
        total_queue_length += len(stopped[link])
    sum_queue += total_queue_length
    queue_samples += 1
...
print("=" * 50)
print(f"\nGrid Configuration:")
print(f"  Size: {N}x{N} grid")
print(f"  Duration: {TOTAL_TICKS} time ticks")

print(f"\nTraffic Light Configuration:")
print(f"  N/S green phase: {CYCLE_NS_GREEN} ticks")
print(f"  E/W green phase: {CYCLE_EW_GREEN} ticks")
print(f"  Total cycle: {CYCLE_TOTAL} ticks")

print(f"\nArrival Configuration:")
print(f"  Arrival rate: {ARRIVAL_RATE} cars/tick at boundary")

print(f"\nPerformance Metrics:")
print(f"  Total trips completed: {completed}")
print(f"  Throughput: {completed/TOTAL_TICKS:.4f} cars/tick")

if completed > 0:
    mean_travel_time = sum_tt / completed
    print(f"  Mean travel time: {mean_travel_time:.2f} ticks")
else:
    print(f"  Mean travel time: N/A (no completed trips)")

if queue_samples > 0:
    mean_queue_length = sum_queue / queue_samples
    print(f"  Mean queue length: {mean_queue_length:.2f} cars")
else:
    print(f"  Mean queue length: N/A")
